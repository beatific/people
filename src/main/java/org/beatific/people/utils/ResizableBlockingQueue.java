...
public class ResizableBlockingQueue<E> extends AbstractQueue<E> implements BlockingQueue<E> {
	...
	/**
	 * Creates a {@code ResizableBlockingQueue} with a capacity of
	...
	/**
	 * Creates a {@code ResizableBlockingQueue} with the given (fixed) capacity.
	 *
	 * @param capacity
	 ...
	 * always equal to the initial capacity of this queue less the current
	 ...
	 * do so immediately without exceeding the queue's capacity, returning
	 * {@code true} upon success and {@code false} if this queue is full. When using
	 * a capacity-restricted queue, this method is generally preferable to method
	 * {@link BlockingQueue#add add}, which can fail to insert an element only by
	...
	private class Itr implements Iterator<E> {
		/*
		 * Basic weakly-consistent iterator. At all times hold the next item to hand out
		 * so that if hasNext() reports true, we will still have it to return even if
		 ...
		 *
		 * Unlike other traversal methods, iterators need to handle both: - dequeued
		 * nodes (p.next == p) - (possibly multiple) interior removed nodes (p.item ==
		 * null)
		 */
	...
	static final class LBQSpliterator<E> implements Spliterator<E> {
		...
		final ResizableBlockingQueue<E> queue;
		Node<E> current; // current node; null until initialized
		int batch; // batch size for splits
		boolean exhausted; // true when no more nodes
		long est; // size estimate

		LBQSpliterator(ResizableBlockingQueue<E> queue) {
			this.queue = queue;
			this.est = queue.size();
		}
		...
		public Spliterator<E> trySplit() {
			Node<E> h;
			final ResizableBlockingQueue<E> q = this.queue;
			int b = batch;
	 ...
	 * @serialData The capacity is emitted (int), followed by all of its elements
	 *             (each an {@code Object}) in the proper order, followed by a null
	 */
